					...
					driver.findElement(By.xpath("//*[@id=\"contents\"]/div[2]/form/fieldset/div[2]/div[1]/select/option["+ index +"]")).click();
						...
						if (index == 12) {
							...
							for(int indexOfTSI = 3; indexOfTSI <= 7; indexOfTSI++) {
								TSI[indexOfTSI - 3] = calculate_TSI(driver.findElement(By.xpath("//*[@id=\"mySheet1\"]/tbody/tr[3]/td/div/div[1]/table/tbody/tr["+ indexOfTSI +"]/td[6]")).getText().replace(",", ""),
...
//					            driver.quit();
	...
	// 무역특화지수 계산
	static String calculate_TSI (String exportV, String importV) { 
	...
	// 현시비교우위지수
	static String calculate_RCA(String countrysItemExports, String countrysTotalExports, String globalsItemExports, String globalTotalExports) {
		
		double KoreaIE = Double.parseDouble(countrysItemExports);
		double KoreaTE = Double.parseDouble(countrysTotalExports);
		double GIE = Double.parseDouble(globalsItemExports);
		double GTE = Double.parseDouble(globalTotalExports);
		
		double calculateRCA = (KoreaIE / KoreaTE) / (GIE / GTE);
	...
	// 현시비교우위지수 등급분류
	// 0 ~ 2.5
	static int[] RCA_rating(String[] RCA) {
